...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Gestionar Atención' }];
...
export default function FormAtencion({ atencion }: { atencion: Atencion }) {
    const isEdit = !!atencion?.id;

    const { data, setData, post, put, processing, errors } = useForm<Atencion>({
        title: atencion.title || '',
        icon: atencion.icon || '',
        color: atencion.color || '',
        descripcion: atencion.descripcion || '',
        activo: atencion.activo || false,
    });
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        const successCallback = () => {
            Swal.fire({
                icon: 'success',
                title: isEdit ? 'Atención actualizada' : 'Atención creada',
                text: isEdit
                    ? 'La atención ha sido actualizada correctamente.'
                    : 'La atención ha sido creada correctamente.',
                timer: 2000,
        ...
        if (isEdit) {
            put(route('cms.atencion.update', atencion.id), {
                onSuccess: successCallback,
            });
        } else {
            post(route('cms.atencion.store'), {
                onSuccess: successCallback,
            });
        }
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={isEdit ? 'Editar Atención' : 'Crear Atención'} />
            ...
            <div className="md:p-15 p-10 bg-white dark:bg-neutral-800">
                <div className="flex items-center justify-between mb-6">
                    <h1 className="text-2xl font-semibold text-gray-900 dark:text-neutral-100">
                        {isEdit ? 'Editar Atención' : 'Crear Nueva Atención'}
                    </h1>
                    <Link href={route('cms.atencion.index')}>
