...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Gestionar Quienes' }];
...
export default function FormQuien({ quien }: { quien: Quien }) {
    const isEdit = !!quien?.id;

    const { data, setData, post, put, processing, errors } = useForm<Quien>({
        title: quien?.title ?? '',
        icon: quien?.icon ?? '',
        color: quien?.color ?? '',
        descripcion: quien?.descripcion ?? '',
        activo: quien?.activo ?? false,
    });
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        if (isEdit) {
            put(route('cms.quienes.update', quien.id), {
                onSuccess: successCallback,
            });
        } else {
            post(route('cms.quienes.store'), {
                onSuccess: successCallback,
            });
        }
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={isEdit ? 'Editar Quien' : 'Crear Quien'} />
            ...
            <div className="md:p-15 p-10 bg-white dark:bg-neutral-800">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <Link href={route('cms.quienes.index')}>
