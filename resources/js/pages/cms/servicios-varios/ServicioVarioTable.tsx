...
interface ServicioVarioTableProps {
    servicios: PaginatedServiciosVarios;
    resourceRoute: string;
}

export default function ServicioVarioTable({ servicios, resourceRoute }: ServicioVarioTableProps) {
    const [localItems, setLocalItems] = useState<ServicioVario[]>(servicios.data);

    useEffect(() => {
        setLocalItems(servicios.data);
    }, [servicios.data]);
    ...
    const handleSwitch = async (item: ServicioVario) => {
        try {
    ...
    const onDelete = async (item: ServicioVario) => {
        const r = await Swal.fire({
            title: '¿Estás seguro?',
            html: `Estás a punto de <b class="text-red-500 uppercase">eliminar</b> el siguiente registro: <div class="my-4 font-bold">ID: ${item.id} (${item.title}).</div>Esta acción eliminará el servicio y todas sus imágenes. Esta acción es irreversible.`,
            icon: 'warning',
        ...
        if (r.isConfirmed) {
            // ✅ Usar router.delete de Inertia en lugar de apiDelete
            router.delete(route(`${resourceRoute}.destroy`, item.id), {
                ...
                onSuccess: () => {
                    ...
                    if (localItems.length === 1 && servicios.links.length > 0) {
                        const currentPageIndex = servicios.links.findIndex(link => link.active);
                        if (currentPageIndex > 0 && servicios.links[currentPageIndex - 1].url) {
                            router.visit(servicios.links[currentPageIndex - 1].url);
                        }
                    }
                },
                onError: (errors) => {
            ...
            <div className="bg-white dark:bg-neutral-800 shadow rounded-lg overflow-x-auto">
                <table className="min-w-full table-auto text-left text-sm">
                    ...
                    <tbody>
                        {localItems.length === 0 ? (
                            <tr>
                                <td colSpan={7} className="px-6 py-4 text-center text-slate-400 dark:text-neutral-500">
                                    Sin registros
                                </td>
                            </tr>
                        ) : (
